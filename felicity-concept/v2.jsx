...
// New component for the falling paper effect
const FallingPapers = () => {
  // Create a fixed set of "papers" with random properties
  // Using specific values to ensure consistent server/client rendering or just defining them once
  const [papers, setPapers] = useState([]);

  useEffect(() => {
    const newPapers = Array.from({ length: 30 }).map((_, i) => ({
      id: i,
      left: Math.random() * 100,      // Random horizontal position (0-100%)
      delay: Math.random() * 10,      // Random start delay
      duration: 8 + Math.random() * 7, // Random fall duration (8-15s) for varying speeds
      size: 10 + Math.random() * 15,  // Random size
      rotation: Math.random() * 360,  // Initial rotation
      color: Math.random() > 0.6 ? '#fcec89' : '#ffffff' // Mix of white and theme yellow papers
    }));
    setPapers(newPapers);
  }, []);
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none z-10">
      {papers.map((paper) => (
        <div
          ...
          style={{
            ...
            width: `${paper.size}px`,
            height: `${paper.size * 1.4}px`, // Make them rectangular like paper
            backgroundColor: paper.color,
            transform: `rotate(${paper.rotation}deg)`,
            // The custom animation defined in the style tag below
            animation: `fall ${paper.duration}s linear infinite`,
            // Negative delay makes them appear to have been falling already when page loads
      ...
      <style>{`
        @keyframes fall {
          0% {
            transform: translate(0, -10vh) rotate(0deg) translateX(-20px);
            opacity: 0;
          }
          10% {
            opacity: 0.7;
          ...
          50% {
             transform: translate(25px, 50vh) rotate(180deg) translateX(25px);
          }
          90% {
             opacity: 0.5;
          }
          100% {
            transform: translate(-25px, 110vh) rotate(360deg) translateX(-25px);
            opacity: 0;
          }
        }
      `}</style>
